...
![image-20220524210349577](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/image-20220524210349577.png)

![Picture2.png](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/379a042b9d8df3a96d1ac0f27346718033bf3bfce69731bab52bf6f372b4c8f4-Picture2.png)

​	使用快速幂求x^n,最终n会变成0,所以x^0=1,res最终就为所求值
...
思路:后序遍历顺序为`左->右->根`,已知一个后序遍历序列`[1,3,2,6,5]`,对于二叉搜索树,左子树数值永远小于根节点,右子树数值永远大于根节点,所以我们从下标为0开始遍历序列,寻找值大于根节点的index,可求出左子树为`[1,3,2]`,右子树为`[6]`,判断右子树序列中数值是否都大于根节点,如果都大于,则满足条件.然后继续递归左右子树,当左右子树节点数小于或等于1时,终止
...
>
> 求出这个数 要求时间复杂度O(n),空间复杂度O(1)

思路:异或运算,相同的数字异或结果为0,遍历数组,异或每一项,最后剩下来的就是只出现一次的数
...
1. 异或运算的规则是[相同为0,相异为1],所以对于数组[a,a,b,b,c,c,x,y],数组中各项异或的结果a⊕a⊕b⊕b⊕c⊕c⊕x⊕y为0⊕0⊕0⊕0⊕0⊕0⊕x⊕y，即x⊕y
2.  由于x、y是两个不同的数字,那么他们的二进制中至少有一位不相等,一个为1，一个为0,**所以他们异或的结果中肯定有一位是1**，那么我们只要找到了**不相等的任意一位**,（**而&运算的规则是[同1为1,否则为0]**),就可以根据&运算将x和y分配的两个子数组中
3. 现在要找到x和y中不相等的一位,可以通过**&1**运算,我们设m的初始值为1,通过m&(x⊕y),并让m依次左移,如果求出的值是1,那么可以说明我们找到了x与y中不相等的一位,我们记录此时的m为mark
4. 我们已经求出了mark的值,现在只要循环遍历nums数组,如果`num&mark == 1`,将他们分到left数组,如果`num&mark == 0`,将他们分到right数组,遍历完后,**相同的数字肯定会分配到同一个数组中**(因为相同的数字与mark做运算的结果是相同的),并且**两个只出现一次的数字肯定会分配到不同的数组中**(因为x或者y肯定有一个`&mark`为1,另一个为0)
5. 这样我们让两个数组中的数做异或运算,就可以获得结果值
...
思路:位运算**,异或运算**(相同为0,相异为1)可以求出两数二进制中相同的数字,相当于做无进位和,**&运算左移一位**可以求出两数的进位

![Picture1.png](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/202205252035983.png)
...
var add = function(a, b) {
    ...
    while(b != 0){//一直执行到进位为0，此时无需再运算,直接输出结果就可以了
...
思路:本题的难点在于 **不能使用除法** ，即需要 **只用乘法** 生成数组 B*B* 。根据题目对 B[i]*B*[*i*] 的定义，可列表格，如下图所示。

![Picture1.png](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/202205252114716.png)

根据表格我们可以构造两个dp数组,分别为left和right,`res[i] = left[i] * right[i]`
...
![image-20220614204930639](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/202206142049822.png)

思路:因为初始值是一个二叉搜索树,我们根据中序遍历就可以从小到达遍历节点,所以我们只需要利用中序遍历获取当前节点并记录前一个遍历的节点,然后将两个节点连接起来就可以了,但是还要保存头节点,因为头节点是要与尾节点连接起来的
...
![img](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/202206142053833.png)

思路:遍历链表 将每个节点存入map中,然后再遍历一次链表,将map中的节点连接起来
...
![image-20220620203415154](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/image-20220620203415154.png)

> 思路:就正常逻辑,将首尾空格去除然后判断就行了
...
![image-20220628221410396](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/image-20220628221410396.png)
