...
OK， 现在已经弄清楚了优先级是怎么算的了。但是，还有一个问题，怎么比较两个优先级的高低呢？c**比较规则是: 从左往右依次进行比较 ，较大者胜出，如果相等，则继续往右移动一位进行比较 。如果4位全部相等，则后面的会覆盖前面的**

还要注意的是,如果没有样式指向想要判断的元素,那么这个元素就采用它的父级元素的样式,具体看练习题第二题
...
![img](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/202204181434500.png)
...
![img](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/202204181439934.png)

样式表中没有样式指向span标签,于是采用它的父标签，也就是p的样式
...
![img](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/202204181440116.png)

样式表中只有一个样式指向了span标签，所以span的颜色为**green**

---

第四题

![img](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/202204181441493.png)
...
第二个和第三个优先级相同,优先采用后面的样式(渲染树渲染是从上到下的原则)
...
![img](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/202204181444660.png)

类选择器优先级别高于标签选择器,答案为**blue**
...
![img](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/202204191657913.webp)
...
![img](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/202204191658661.webp)
...
![img](http://image-yunsheng.test.upcdn.net/typora-cloud-img/raw/master/16e83067c347ff13~tplv-t2oaga2asx-zoom-in-crop-mark:1304:0:0:0.awebp)

总结:vue为了减少频繁的dom渲染,在数据改变后,会开启一个队列来缓冲同义事件循环发生的所有数据改变,当下一轮事件循环开启时,将队列清空并进行相应的dom渲染,而nextTick的作用就是在下一轮事件循环中执行回调,在回调中我们可以拿到更新后的dom
